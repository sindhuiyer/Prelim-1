...
#u[1] = x; u[2] = y; u[3] = z
#Use revious steady state value to get the not one
...
#iteration in continuity over the reverse to her the other steady states of X
...
for i  = 1:size(S_rev,1)
    ...
    tspan = (0.0,1000.0)                     #start and end time
    prob = ODEProblem(lorenz!,rev_uval[i,:],tspan)
    sol = solve(prob)                       #Solve the system
